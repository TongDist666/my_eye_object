...
def compute_circle_sum(img,points,center):
    sum_0=0
    for point in points:
...
def compute_circle_sum_list(img,points,center,i,j):
    list_temp=[]
    for i0 in range(i,j+1):
        list_temp.append(compute_circle_sum(img,points[i0],center))
    return list_temp
...
def T_func(img_0,all_circle_dir_0,center_0,rr1,rr2,l_l,flag=0):
    #获取圆心上  各半径圆上像素和  的列表
    list_01=compute_circle_sum_list(img_0,all_circle_dir_0,center_0,rr1,rr2)
    #获取  其差值列表
